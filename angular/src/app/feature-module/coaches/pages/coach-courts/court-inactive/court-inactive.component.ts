...
@Component({
  selector: 'app-court-inactive',
  templateUrl: './court-inactive.component.html',
  styleUrls: ['./court-inactive.component.scss']
})
export class CourtInactiveComponent {
  checked!: boolean;
  ...
  constructor(
    ...
    this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
      if (this.router.url == this.routes.coachinactivecourts) {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getInactiveCourts().subscribe((apiRes: apiResultFormat) => {
      this.tableData = [];
